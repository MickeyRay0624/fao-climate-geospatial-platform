  ...
  return (
    <div className="platform-page wizard-page">
      ...
      <section className="wizard-card">
        ...
        {stage === 3 && <div className="wizard-section"><div className="wizard-title"><span>04</span><div><h2>Select the source file</h2><p>The browser uploads directly to MinIO quarantine using a short-lived signed URL.</p></div></div><label className={`drop-zone ${file ? "has-file" : ""}`}><input type="file" onChange={(event) => setFile(event.target.files?.[0] ?? null)} accept=".geojson,.json,.zip,.gpkg,.csv,.pdf,.docx,.md,.txt" /><span>{file ? "✓" : "⇧"}</span><strong>{file?.name ?? "Choose a file or drop it here"}</strong><small>{file ? `${(file.size / 1024).toFixed(1)} KB · ${file.type || "type unknown"}` : "GeoJSON, Shapefile ZIP, GeoPackage, CSV or document · up to 100 MB"}</small></label><div className="classification-note"><strong>Direct vector controls.</strong><span>A Shapefile ZIP must contain one matching .shp/.shx/.dbf/.prj layer. Multiple layers and non-WGS84 preview transforms are blocked rather than guessed.</span></div><div className="scan-warning"><span>!</span><p><strong>Development scan bypass is active.</strong> The scanner abstraction fails closed outside development; this local file will be visibly marked BYPASSED DEV, never “clean”.</p></div></div>}
