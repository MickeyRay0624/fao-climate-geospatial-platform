...
export default function VersionDetailPage() {
  ...
  return (
    <div className="platform-page detail-page version-page">
      ...
      {activeTab === "Download" && <section className="detail-panel download-panel"><span className="download-icon">⇩</span><div><p className="platform-kicker">Controlled download</p><h2>Request a short-lived asset URL</h2><p>The API evaluates visibility, classification, grants and explicit denies again. The URL expires and permanent object credentials are never sent to the browser.</p><div className="analysis-actions"><button className="platform-primary" disabled={busy || !permission("dataset.download")} onClick={() => void act(async () => { const download = await getVersionDownload(version.id); window.open(download.url, "_blank", "noopener"); }, "A short-lived source URL was issued and audited.")}>Download original source</button>{version.assets.some((asset) => asset.role === "derived-vector") && <button className="platform-secondary" disabled={busy || !permission("dataset.download")} onClick={() => void act(async () => { const download = await getVersionDownload(version.id, "derived-vector"); window.open(download.url, "_blank", "noopener"); }, "A short-lived derived GeoJSON URL was issued and audited.")}>Download derived GeoJSON</button>}</div></div></section>}
    </div>
  );
}
